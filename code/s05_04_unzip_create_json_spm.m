%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                        %
%   Distortion Correction - Unzip FUGUE outputs and create JSON (SPM)    %
%                                                                        %
%   Unzips the new images from the previous script and creates a JSON    %
%   for them (updated from the previous 'ra*' realigned images) - copies %
%   the metadata, appends the information regarding the Distortion       %
%   Correction steps (s05_01 to s05_03), and saves it as the new         %
%   'ura*.json' sidecar.                                                 %
%                                                                        %
...
for s = 1:length(subjects)
    ...
    for p = 1:length(ura_files_gz)
        gz_file = fullfile(func_dir,ura_files_gz(p).name);
        ...
        [~,name_only,~] = fileparts(replace(gz_file, '.nii.gz', '.nii'));

        base_name_raw = eraseBetween(name_only,1,3); % removes 'ura' prefix to obtain the original file name
        run_str = regexp(name_only,'run-\d+','match','once');
    
        ra_json_path = fullfile(func_dir,['ra' base_name_raw '.json']);
        ura_json_path = fullfile(func_dir,[name_only '.json']);
        fprintf('Creating JSON for BIDS compliance...\n');
        create_ura_json(ra_json_path,ura_json_path,subj,run_str);
    end
end
fprintf('\nDone!\n');
...
json_data.B0FieldSource = sprintf('rfmap_rads_%s_%s.nii',subj,run_str);
