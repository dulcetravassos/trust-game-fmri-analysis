...
%                                                                        %
%   This script automates the estimation of group-level models across    %
%   all functional contrasts. It processes both One-Sample and           %
%   Two-Sample (Learners vs Non-Learners) Design Matrices (SPM.mat)      %
%   generated in the previous steps and runs the estimation algorithm    %
...
% Model 1: One-Sample T-Test
models(1).type = 'One-Sample';
models(1).dir = fullfile(deriv_dir,'spm-statistics','2nd-level');
models(1).cons = {'sc_1', 'sc_2', '1a', '1b', '2a', '2b', '3', '4a', '4b', ...
    '4c', '4d', '5a', '5b', '5c', '6a', '6b', '6c', '7', '8a', '8b', ...
    '9a', '9b', '9c', '9d', '10a', '10b', '10c'};

% Model 2: Two-Sample T-Test
models(2).type = 'Two-Sample';
models(2).dir = fullfile(deriv_dir,'spm-statistics','2nd-level_TwoSample');
models(2).cons = {'2a','2b','3','6a','6b','6c','7'};
...
for m = 1:length(models)
        current_model_type = models(m).type;
        current_model_dir = models(m).dir;
        current_cons = models(m).cons;

        fprintf('\n==================================================\n');
        fprintf('Starting Beta estimation for %s Models\n',current_model_type);
        fprintf('==================================================\n');

    for c = 1:length(current_cons)
        con = current_cons{c};
        fprintf('\n==================================================\n');
        fprintf('Beta estimation for contrast: %s\n', con);
    
        stats_dir = fullfile(current_model_dir,con);
