%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                        %
%   Smoothing (BIDS ready)                                               %
%                                                                        %
%   Applies a spatial Gaussian filter to the normalized functional       %
%   images (wura*), increasing the Signal-to-Noise Ratio and             %
%   accommodating anatomical variations between subjects for group       %
%   stats. Outputs are saved with an 's' prefix (swura*) and JSON        %
%   sidecars are created accordingly.                                    %
%                                                                        %
...
% Rule of thumb: ~2x the voxel size
fwhm_kernel = [5 5 6];
...
for s = 1:length(subjects)
    ...
    for t=1:length(tasks)
        ...
        % Get functional files (wura* files)
        file_pattern = sprintf('wura*%s*_bold.nii',current_task);
        ...
        % ####################### SPM Batch #######################
        clear matlabbatch;
        matlabbatch{1}.spm.spatial.smooth.data = func_vols;
        ...
        try
            ...
            for r=1:length(run_files)
                ...
                new_func_name = ['s' orig_func_name];
                ...
                create_smooth_json(source_json,target_json,fwhm_kernel);
            end
...
function create_smooth_json(source_json_path,target_json_path,fwhm_kernel)
...
json_data.SpatialSmoothing = true;
json_data.SmoothingFWHM = fwhm_kernel;
json_data.SmoothingSoftware = 'SPM12';
json_data.Description = 'Spatial Smoothing applied using a Gaussian kernel';
